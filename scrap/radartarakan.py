...
RADAR_GRAPHQL_URL = "https://api-jjmn.jawapos.com/api-jp-graphql/"
RADAR_PUBLISHER_ID = "10"
RADAR_BASE_URL = "https://radartarakan.jawapos.com"
RADAR_QUERY = """
...
def slugify_title(title):
    """Build a URL-safe slug using the same rules as the Radar Tarakan frontend."""
...
def build_article_url(article):
    ...
    return f"{RADAR_BASE_URL}/{category_slug}/{article_id}/{title_slug}"


def fetch_radar_page(start_date, end_date, page=1, per_page=20):
    """Fetch one Radar Tarakan index page through its GraphQL API."""
    payload = {
        "operationName": "GET_SEARCH_ARTICLE",
        "query": RADAR_QUERY,
        "variables": {
            "publisherId": RADAR_PUBLISHER_ID,
            "page": page,
            "perPage": per_page,
    ...
    response = SESSION.post(
        ...
        headers={
            ...
            "Origin": RADAR_BASE_URL,
            "Referer": f"{RADAR_BASE_URL}/indeks-berita",
        },
    )
...
def extract_radar_content(url):
    """Fetch and normalize article body text from a Radar Tarakan detail page."""
    try:
        ...
        print(f"[WARN] Gagal mengambil isi Radar Tarakan: {url} -> {e}")
        return None
...
def radar_scrap(start_date, end_date, progress_callback=None):
    """Scrape Radar Tarakan via the GraphQL endpoint used by the current frontend."""
    scraped_data = []
    current_page = 1
    ...
    while True:
        if progress_callback:
            ...
            progress_callback(
                page_progress,
                f"Memuat halaman API {current_page} Radar Tarakan.",
            )

        try:
            payload = fetch_radar_page(start_date, end_date, page=current_page)
        except Exception as e:
            raise RuntimeError(f"Gagal mengambil data Radar Tarakan dari API: {e}") from e

        if payload.get("errors"):
            raise RuntimeError(f"GraphQL Radar Tarakan mengembalikan error: {payload['errors']}")
        ...
        for article_index, article in enumerate(articles, start=1):
            ...
            scraped_data.append({
                ...
                "Isi": extract_radar_content(article_url),
            })
            ...
            if progress_callback and (article_index == total_articles or article_index % 5 == 0):
                ...
                progress_callback(
                    article_progress,
                    f"{len(scraped_data)} berita Radar Tarakan sudah terkumpul.",
                )
    ...
    if progress_callback:
        progress_callback(1.0, f"Selesai. {len(scraped_data)} berita Radar Tarakan terkumpul.")
